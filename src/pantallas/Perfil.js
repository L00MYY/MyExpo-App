...
export default function Perfil({ navigation }) {
    ...
    const fetchProfileData = async () => {
        try {
            const response = await fetch(`${ip}/expo24/api/services/serviceProfesores/profesor.php?action=readProfile`, {
                method: 'POST'
            });
    ...
    const editProfile = async () => {
        try {
            ...
            formData.append('nombreCliente', profileData.nombre_profesor);
            formData.append('apellidoCliente', profileData.carnet_profesor);
            formData.append('aliasCliente', profileData.correo_profesor);
    ...
    return (
        <View style={styles.container}>
            <TextInput
                ...
                onChangeText={(text) => setProfileData((prevData) => ({ ...prevData, nombre_profesor: text }))}
            />
            <TextInput
                style={styles.input}
                placeholder="Carnet"
                value={profileData.carnet_profesor}
                onChangeText={(text) => setProfileData((prevData) => ({ ...prevData, carnet_profesor: text }))}
            />
            <TextInput
                style={styles.input}
                placeholder="Correo electrónico"
                value={profileData.correo_profesor}
                onChangeText={(text) => setProfileData((prevData) => ({ ...prevData, correo_profesor: text }))}
            />
